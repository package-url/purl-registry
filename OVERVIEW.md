...
- Use [Package-URL (PURL)](https://github.com/package-url/purl-spec) to 
create an open and distributed registry of
C/C++ packages keyed by PURL, with associated metadata, but neutral
regarding any build system;
- Maintain open source tools to discover and detect C/C++ code that is 
commonly vendored and patched in software codebases, and
- Maintain a database of known security vulnerabilities that affect
these C/C++ packages, also keyed by PURL.

With this data software teams will be able to more efficiently and reliably 
manage and automate their software supply chain and vulnerability management 
operations for C/C++ code to improve their security posture and regulatory 
security compliance, using open data and open code.

## Context

Most software development teams integrate ever more open source code in
their projects or products. A large number of software products, and hardware
...
CycloneDX SBOM formats. PURL is an Ecma standard: [ECMA-427](ttps://tc54.org/purl) 
...
i.e., across the entire software lifecycle.
...
databases. Components may be vulnerable but queries without accurate
...
- Do not have an obvious and easily inferred PURL software identifier,
- Do not have package manifests and structured metadata, and
- Are scattered across multiple different internet locations, instead of a 
single, centralized or coordinated packaging ecosystem.
...
-  he code is commonly copied and vendored in code trees that also
contain proprietary code, making third-party, vendored code
difficult to distinguish from proprietary code;
- The C/C++ code is commonly modified (perhaps minimally) to apply
...
PURL identification and discovery of C/C++ software. The high level
...
on the most popular packages and the most common packages in use by
...
usage. The steps for this include new code and data:
   ...
   - Populate the PurlDB for these packages by running its data mining
   code,
   ...
   the tools to populate these from the PurlDB and import its data into a
...
3. Enable content-based discovery and identification of C/C++ packages. The 
goal is to enable finding existing C/C++ packages when scanning a product 
codebase with:
...
exactly and approximately using fingerprints when scanning a codebase. We will continuously update and release an index of the C/C++ packages available in 
the registry for easy deployment of local code matching.
...
metadata,
   - Create a ScanCode.io pipeline for C/C++ codebase analysis, including 
metadata collection and code matching,
   - Update a MatchCode matching pipeline for C/C++ support using the PurlDB
   catalog, and
   - Create documentation.
|
4. Extend the ABOUT file format to serve as the “missing” metadata manifest
   ...
   See the section“ABOUT file metadata format” for details and examples. The 
   high-level steps include:

   - Document key fields and their usage,
   - Update AboutCode Toolkit with latest PURL support, and
   - Add capability to export ABOUT files from PurlDB.
|
5. Create an open mapping between CVEs and C/C++ PURLs to enable known
   ...
   vulnerability database. Note that CVE.org has recently adopted PURL as a 
   key identifier. The high-level steps include:
   ...
   to support the accurate creation of relationships between
...
2) Ensure SBOM and PURL accuracy for C/C++ packages.
   ...
   PURL accuracy is essential to enable the safe reuse of FOSS. With a
valid PURL, users can find accurate and timely data related to
...
and identified accurately, and that known vulnerabilities are found for
the correct package.
